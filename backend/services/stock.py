...
def overview() -> list[dict]:
    """Every package and where all of its vouchers are.

    The office view alone is misleading once sheets are issued straight to
    agents: the shelf reads empty while a hundred vouchers are out in the
    field. This counts all four states so the totals actually add up.
    """
    return rows(
        "SELECT p.id, p.name, p.price, p.colour, p.validity, p.active, "
        " COUNT(v.id) AS total, "
        " COALESCE(SUM(v.status = 'office'),0) AS office_qty, "
        " COALESCE(SUM(v.status = 'agent'),0)  AS agent_qty, "
        " COALESCE(SUM(v.status = 'sold'),0)   AS sold_qty, "
        " COALESCE(SUM(v.status = 'dead'),0)   AS void_qty, "
        " COALESCE(SUM(CASE WHEN v.status='office' THEN v.price ELSE 0 END),0) AS office_value, "
        " COALESCE(SUM(CASE WHEN v.status='agent'  THEN v.price ELSE 0 END),0) AS agent_value, "
        " COALESCE(SUM(CASE WHEN v.status='sold'   THEN v.price ELSE 0 END),0) AS sold_value, "
        " COALESCE(SUM(CASE WHEN v.status='dead'   THEN v.price ELSE 0 END),0) AS void_value, "
        " COUNT(DISTINCT v.price) AS price_runs, "
        # The price the cards were actually registered at, which is what they
        # are worth. The package price is only what the NEXT batch will cost.
        " MIN(v.price) AS low_price, MAX(v.price) AS high_price "
        "FROM packages p LEFT JOIN vouchers v ON v.package_id = p.id "
        "GROUP BY p.id ORDER BY p.sort_order, p.price")


def in_the_field() -> list[dict]:
    """What each agent is holding, and what she has sold."""
    return rows(
        "SELECT a.id, a.code, a.name, a.station, "
        " COALESCE(SUM(v.status = 'agent'),0) AS holding, "
        " COALESCE(SUM(CASE WHEN v.status='agent' THEN v.price ELSE 0 END),0) AS holding_value, "
        " COALESCE(SUM(v.status = 'sold'),0) AS sold, "
        " COALESCE(SUM(CASE WHEN v.status='sold' THEN v.price ELSE 0 END),0) AS sold_value, "
        " COALESCE(SUM(v.status = 'dead'),0) AS void "
        "FROM agents a LEFT JOIN vouchers v ON v.agent_id = a.id "
        "WHERE a.active = 1 GROUP BY a.id ORDER BY holding_value DESC, a.name")


def totals() -> dict:
    lines = overview()
    return {
        "registered": sum(l["total"] for l in lines),
        "office_qty": sum(l["office_qty"] for l in lines),
        "agent_qty": sum(l["agent_qty"] for l in lines),
        "sold_qty": sum(l["sold_qty"] for l in lines),
        "void_qty": sum(l["void_qty"] for l in lines),
        "office_value": sum(l["office_value"] for l in lines),
        "agent_value": sum(l["agent_value"] for l in lines),
        "sold_value": sum(l["sold_value"] for l in lines),
        "void_value": sum(l["void_value"] for l in lines),
    }
