...
def all_agents(include_inactive: bool = False) -> list[dict]:
    """Every agent, with the name of the building they cover."""
    sql = ("SELECT a.*, b.name AS building, b.area AS building_area "
           "FROM agents a LEFT JOIN buildings b ON b.id = a.building_id")
    if not include_inactive:
        sql += " WHERE a.active = 1"
    return rows(sql + " ORDER BY a.name")


def get(agent_id: int) -> dict | None:
    return one_dict(
        "SELECT a.*, b.name AS building, b.area AS building_area "
        "FROM agents a LEFT JOIN buildings b ON b.id = a.building_id "
        "WHERE a.id = ?", (agent_id,))
...
def create(code: str, name: str, phone: str = "", station: str = "",
           ...
           next_of_kin: str = "", notes: str = "",
           building_id: int | None = None) -> int:
    ...
    return execute(
        ...
        (code, name, phone.strip(), station.strip(), commission_rate,
         ...
         nin.strip(), next_of_kin.strip(), notes.strip(), building_id or None),
    )
...
def update(agent_id: int, **fields) -> None:
    ...
    execute(
        ...
        "next_of_kin = ?, notes = ?, active = ?, building_id = ? WHERE id = ?",
        (str(fields["name"]).strip(), str(fields.get("phone", "")).strip(),
         ...
         fields.get("building_id") or None, agent_id),
    )
