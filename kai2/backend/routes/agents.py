...
def delete(agent_id: int):
    ...
    if agent is None:
        ...
        return redirect(url_for("agents.index"))
    try:
        outcome = agent_service.delete(agent_id)
    except ValueError as exc:
        flash(str(exc), "error")
        return redirect(url_for("agents.profile", agent_id=agent_id))
    auth.audit(f"agent-{outcome}", agent["code"])
    if outcome == "deleted":
        flash(f"{agent['name']} was removed.", "ok")
    else:
        flash(f"{agent['name']} has stock or money on record, so they were "
              f"switched off rather than deleted. Their history stays intact.",
              "warn")
    return redirect(url_for("agents.index"))
